    ...
    methods: {
        fetchData(url) {
            fetch(url).then(response => response.json()).then(data => {
                if (window.location.pathname === "/" || window.location.pathname === "/2index.html") {
                    this.categories = data.categories ? data.categories : ["Drinks", "Meals"];
                    this.categoriesBK = data.categories ? data.categories : ["Drinks", "Meals"];
                }
            })
        },
        fetchDataDrinks(url) {
    ...
    computed: {
        filteredItems() {
            ...
            if (this.selectedCategories.length > 0) {
                if (!this.selectedCategories.includes("Drinks")) {
                    filteredDrinks = [];
                }
                if (!this.selectedCategories.includes("Meals")) {
                    filteredMeals = [];
                }
            }
